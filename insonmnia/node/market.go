...
func (m *marketAPI) execute(handler *orderHandler) error {
	...
	matcher := dealer.NewBidMatcher()
	askToPropose, err := matcher.Match(orders)
	if err != nil {
	...
	dealr := dealer.NewDealer(m.remotes.key, hubClient, m.remotes.eth, m.remotes.dealCreateTimeout)
	dealID, err := dealr.Deal(m.ctx, handler.order, askToDealWith)
	if err != nil {
		return err
	}
